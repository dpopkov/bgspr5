...
public abstract class BaseMusicService<
    ...
    protected BaseMusicService(BaseArtistRepository<A, ID> artistRepository, BaseSongRepository<A, S, ID> songRepository, WildCardConverter converter) {
    ...
    @Transactional
    public Optional<A> getArtistById(ID id) {
        return artistRepository.findById(id);
    }

    @Transactional
    public Optional<S> getSongById(ID id) {
    ...
    public List<String> getMatchingSongNamesForArtist(String artistName, String songTitle) {
        A artist = getArtist(artistName);
        return songRepository.findByArtistIdAndNameLikeIgnoreCaseOrderByNameDesc(
                artist.getId(),
