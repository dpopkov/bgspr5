...
public class ArtistControllerTest extends AbstractTestNGSpringContextTests {
    ...
    public void testGetArtistById(int id, String name) {
        ...
        if (id != -1) {
            ...
            Artist expected = new Artist(id, name);
            assertEquals(artist, expected);
        } else {
    ...
    /*
     * This method tries to save an artist that should already exist in the database;
     * this will validate that the Repository's saveArtist() method returns a valid artist
    ...
    public void testSearches(String name, int count) {
        // this is used to help Spring figure out what types are returned by restTemplate.exchange()
    ...
    /*
     * We need this to run AFTER testSearches completes, because testSaveArtist() adds to the artist list
     * and therefore we might get one more artists than we're expecting out of some searches.
     */
